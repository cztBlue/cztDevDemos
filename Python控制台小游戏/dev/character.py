...
class Player(Character):
    ...
    def load(data):
        return Player(
            prefabname=data[1],
            position=Vector2(listinit=data[3]),
            health=data[4],
            mana=data[5],
            level=data[6],
            exp=data[7],
            backpack=Backpack.load(data[9]),
        )
    ...
    def create_self_by_vec_icon(vec, icon):
        return Player(icon=icon, position=vec)
...
class Enemy(Character):
    ...
    @staticmethod
    def create_self_by_vec_icon(vec, icon):
        mapp = gvar.otprefabinfo
        return Enemy(
            prefabname=mapp[icon]["prefabname"],
            icon=icon,
            position=vec,
            health=mapp[icon]["basehealth"],
            mana=mapp[icon]["basemana"],
            level=1,
            exp=mapp[icon]["exp"],
            basemana=mapp[icon]["basemana"],
            baseattack=mapp[icon]["baseattack"],
            basedefense=mapp[icon]["basedefense"],
            basehealth=mapp[icon]["basehealth"],
        )
